...
#DATASET_PATH=${ROOT_DIR}/../dataset/sample_long_32k_sft/sample_long_sft_32k_48M.json
DATASET_PATH=${DATASET_PATH:-"${STORAGE_PATH}/dataset/sample_long_32k_sft/qwen25_7b_sample_long_sft_32k_pack_text_document"}
...
DETAIL_TASK_NAME="${TASK_NAME}-lr-${LR}-minlr-${MIN_LR}-bs-${MICRO_BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE}-seqlen-${SEQ_LEN}-pr-${PR}-tp-${TP}-pp-${PP}-cp-${CP}-virtual_pp-${VIRTUAL_PP}-ac-${AC}-do-${DO}-sp-${SP}/${CURRENT_TIME}${LABEL}"
