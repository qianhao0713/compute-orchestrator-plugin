...
if [[ $SEQ_LEN_MODE == "32k" ]]; then
    if [[ $WORLD_SIZE -eq 32 ]]; then
        ...
        PRESET_ETP=1
        PRESET_CP=1
        PRESET_LOAD_OPTI=false
        PRESET_PIPELINE_LAYOUT="Et*3|(t*4|)*22t*3L"
        PRESET_SEQ_LEN=32768
        PRESET_GLOBAL_BATCH_SIZE=256
        PRESET_DEFER_EMBEDDING_WGRAD_COMPUTE=true
        PRESET_WGRAD_DEFERRAL_LIMIT=2
        ...
        PRESET_CP=1
        PRESET_LOAD_OPTI=false
        PRESET_PIPELINE_LAYOUT="Et*1|(t*2|)*46t*1L"
        PRESET_SEQ_LEN=32768
        PRESET_GLOBAL_BATCH_SIZE=128
        PRESET_DEFER_EMBEDDING_WGRAD_COMPUTE=true
        PRESET_WGRAD_DEFERRAL_LIMIT=4
        ...
        PRESET_PIPELINE_LAYOUT="Et*1|(t*2|)*46t*1L"
        PRESET_SEQ_LEN=32768
        PRESET_GLOBAL_BATCH_SIZE=64
    elif [[ $WORLD_SIZE -eq 2 ]]; then
        export XMLIR_ENABLE_NEW_PG=0  # 2机新pg hang
        # 模型层数
        PRESET_SEQ_LEN=32768
        PRESET_NUM_LAYERS=4
        # 并行配置
        ...
        PRESET_ETP=1
        PRESET_CP=1
        PRESET_DIST_CKPT_STRICTNESS=log_unexpected
        PRESET_GLOBAL_BATCH_SIZE=8
...
if [[ "$PRESET_PIPELINE_LAYOUT" != "" ]]; then
    PIPELINE_LAYOUT=${PIPELINE_LAYOUT:-${PRESET_PIPELINE_LAYOUT}}
fi

if [[ -n "${PRESET_NUM_LAYERS}" ]]; then
    NUM_LAYERS=${NUM_LAYERS:-${PRESET_NUM_LAYERS}}
fi

if [[ "$PRESET_DIST_CKPT_STRICTNESS" != "" ]]; then
    DIST_CKPT_STRICTNESS=${DIST_CKPT_STRICTNESS:-${PRESET_DIST_CKPT_STRICTNESS}}
fi

if [[ -n "${PRESET_DEFER_EMBEDDING_WGRAD_COMPUTE}" ]]; then
    DEFER_EMBEDDING_WGRAD_COMPUTE=${DEFER_EMBEDDING_WGRAD_COMPUTE:-${PRESET_DEFER_EMBEDDING_WGRAD_COMPUTE}}
fi

if [[ -n "${PRESET_WGRAD_DEFERRAL_LIMIT}" ]]; then
    WGRAD_DEFERRAL_LIMIT=${WGRAD_DEFERRAL_LIMIT:-${PRESET_WGRAD_DEFERRAL_LIMIT}}
fi

LOAD_OPTI=${LOAD_OPTI:-${PRESET_LOAD_OPTI}}

### OTHERS ###
export BKCL_RING_BUFFER_SIZE=${BKCL_RING_BUFFER_SIZE:-8388608}
ENABLE_GROUPED_GEMM=${ENABLE_GROUPED_GEMM:-true}
...
DATASET_PATH=${DATASET_PATH:-${STORAGE_PATH}/dataset/sample_long_32k_sft/qwen3_235b_sample_long_sft_32k_pack_text_document}
PRETRAIN_CHECKPOINT_PATH=${PRETRAIN_CHECKPOINT_PATH:-${STORAGE_PATH}/mbridge_ckpt/Qwen3-235B-A22B-mcore}
...
if [[ -n "$PIPELINE_LAYOUT" ]]; then
    TRAINING_ARGS+=(--pipeline-model-parallel-layout "$PIPELINE_LAYOUT")
fi
...
if [ ${DEFER_EMBEDDING_WGRAD_COMPUTE:-false} = true ]; then
    TRAINING_ARGS+=(
        --defer-embedding-wgrad-compute
        --wgrad-deferral-limit ${WGRAD_DEFERRAL_LIMIT:-2}
    )
fi

if [ ${NO_ALIGN_GRAD_REDUCE_PARAM_GATHER:-false} = true ]; then
    TRAINING_ARGS+=(
...
CACHE_BASE_DIR=${CACHE_BASE_DIR:-${OUTPUT_BASEPATH}}
...
if [ -n "${DIST_CKPT_STRICTNESS}" ]; then
    TRAINING_ARGS+=(
...
if [[ ${BENCHMARK_MOE_ROUTER_FORCE_LB:-false} = true ]] || [[ ${BENCHMARK_MOE_ROUTER_FORCE_LB:-false} = 1 ]]; then
    DEBUG_ARGS+=(--moe-router-force-load-balancing)
fi

if [[ ${PROFILER_DEBUG:-false} = true ]] || [[ ${PROFILER_DEBUG:-false} = 1 ]]; then
    DEBUG_ARGS+=(
        --profile
        --use-pytorch-profiler
        --profile-step-start ${PROFILE_STEP_START:-3}
...
set -x
env | tee -a ${LOG_FILE}
python -m torch_xmlir --doctor | tee -a ${LOG_FILE}
