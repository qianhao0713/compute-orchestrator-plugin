...
PRETRAIN_CHECKPOINT_PATH=${PRETRAIN_CHECKPOINT_PATH:-"${STORAGE_PATH}/model/Meta-Llama-3-8B"}
# PRETRAIN_CHECKPOINT_PATH=${ROOT_DIR}/../ckpt/Meta-Llama-3-8B-mcore-TP-${TP}-PP-${PP}
DATASET_PATH=${DATASET_PATH:-"${STORAGE_PATH}/dataset/sample_long_32k_sft/llama3_8b_sample_long_sft_32k_pack_text_document"}
...
data_cache_options=" \
        --data-cache-path $OUTPUT_BASEPATH/llama3_${MODEL_SIZE}/data_cache"
...
DETAIL_TASK_NAME="${TASK_NAME}-lr-${LR}-minlr-${MIN_LR}-bs-${MICRO_BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE}-seqlen-${SEQ_LEN}-pr-${PR}-tp-${TP}-pp-${PP}-cp-${CP}-virtual_pp-${VIRTUAL_PP}-ac-${AC}-do-${DO}-sp-${SP}/${CURRENT_TIME}"
