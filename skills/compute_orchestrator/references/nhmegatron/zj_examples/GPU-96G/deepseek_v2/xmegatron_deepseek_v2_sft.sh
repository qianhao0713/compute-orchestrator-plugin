...
if [[ $WORLD_SIZE -eq 32 ]]; then
    ...
    export MOE_LAYERS=${MOE_LAYERS:-59}
    # 并行配置
    export TP=${TP:-2}
    export PP=${PP:-8}
    export EP=${EP:-32}
    export ETP=${ETP:-1}
    export CP=${CP:-1}
    export PIPELINE_LAYOUT=${PIPELINE_LAYOUT:-"Et*6(|t*7)*2(|t*8)*5L"}
    # 训练参数
    export SEQ_LEN=${SEQ_LEN:-32768}
    export GLOBAL_BATCH_SIZE=${GLOBAL_BATCH_SIZE:-256}
    export AC=${AC:-full}
    export PRETRAIN_CHECKPOINT_PATH="/mnt/lhycpfs/lhy/mbridge_ckpt/DeepSeek-V2-mcore"
    ...
    export DENSE_LAYERS=${DENSE_LAYERS:-1}
    export MOE_LAYERS=${MOE_LAYERS:-5}
    # 并行配置
    export TP=${TP:-2}
    export PP=${PP:-1}
    export EP=${EP:-16}
...
SAVE_PATH="$OUTPUT_DIR/checkpoint/mcore-deepseek-v2-236B"
...
MODEL_ARGS=(
    --model-config deepseek_v2_config
    --moe-grouped-gemm
