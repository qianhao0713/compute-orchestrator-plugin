...
from megatron_patch.arguments import core_transformer_config_from_args, get_patch_args
...
def forward_step(data_iterator, model: GPTModel):
    ...
    tokens, labels, loss_mask, attention_mask, position_ids, _, packed_seq_params = get_batch(
        data_iterator
    )
...
def is_dataset_built_on_rank():
    """Check if the dataset is built on this rank."""
    return (
...
def core_gpt_dataset_config_from_args(args):
    """Build the GPT dataset config from the arguments."""
...
if __name__ == "__main__":
    megatron_plugin_init(use_version="0.10.0")
