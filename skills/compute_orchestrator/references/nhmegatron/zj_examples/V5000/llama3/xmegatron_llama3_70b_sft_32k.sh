...
export PYTHONPATH=${PYTHONPATH}:${WORKSPACE_DIR}/Megatron-LM/${MCORE_VERSION:-core_r0_10_0}:${ROOT_DIR}/megatron_patch/${MEGATRON_PATCH_VERSION:-megatron_patch_241113}
...
PRETRAIN_CHECKPOINT_PATH=${PRETRAIN_CHECKPOINT_PATH:-"${STORAGE_PATH}/model/Meta-Llama-3-70B"}
# PRETRAIN_CHECKPOINT_PATH=${ROOT_DIR}/../ckpt/Meta-Llama-3-70B-mcore-TP-${TP}-PP-${PP}
DATASET_PATH=${DATASET_PATH-"${STORAGE_PATH}/dataset/sample_long_32k_sft/llama3_70b_sample_long_sft_32k_pack_text_document"}
...
data_cache_options=" \
        --data-cache-path $OUTPUT_BASEPATH/llama3_70b/data_cache"
...
DETAIL_TASK_NAME="${TASK_NAME}-lr-${LR}-minlr-${MIN_LR}-bs-${BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE}-seqlen-${SEQ_LEN}-pr-${PR}-tp-${TP}-pp-${PP}-cp-${CP}-virtual_pp-${VIRTUAL_PP}-ac-${AC}-do-${DO}-sp-${SP}/${CURRENT_TIME}"
...
run_cmd="env;torchrun $DISTRIBUTED_ARGS pretrain_llama.py
 ${megatron_options} \
 ${save_ckpt_options} \
 ${pr_options} \
 ${load_options} \
