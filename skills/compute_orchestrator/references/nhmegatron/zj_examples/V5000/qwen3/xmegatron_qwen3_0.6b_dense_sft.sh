...
DATA_PATH=${DATASET_PATH:-${STORAGE_PATH}/dataset/sample_long_0.6k_sft/qwen3_0.6b_sample_long_sft_32k_pack_text_document}

PRETRAIN_CHECKPOINT_PATH=${PRETRAIN_CHECKPOINT_PATH:-${STORAGE_PATH}/mbridge_ckpt/Qwen3-0.6B-mcore}
...
MP_SFT_PACKING=${MP_SFT_PACKING:-true}
DATASET_SPLIT=${DATASET_SPLIT:-"969,30,1"}
UNTIE_EMBEDDINGS=${UNTIE_EMBEDDINGS:-""}  # 留空表示根据MODEL_SIZE自动决定，可设置true/false强制启用或禁用
...
# 根据UNTIE_EMBEDDINGS配置决定是否使用--untie-embeddings-and-output-weights
# 留空: 根据MODEL_SIZE自动决定（使用PRESET_UNTIE_EMBEDDINGS值）
# true: 强制启用
# false: 强制禁用
if [ "${UNTIE_EMBEDDINGS}" = "true" ]; then
    MODEL_ARGS+=(--untie-embeddings-and-output-weights)
elif [ "${UNTIE_EMBEDDINGS}" = "false" ]; then
    : # 不添加参数
elif [ -z "${UNTIE_EMBEDDINGS}" ] && [ "${PRESET_UNTIE_EMBEDDINGS}" = "true" ]; then
    MODEL_ARGS+=(--untie-embeddings-and-output-weights)
fi
...
if [ $SFT = true ]; then
    ...
    # 根据MP_DATASET_FORMAT和MP_SFT_PACKING配置数据格式
    if [ "${MP_DATASET_FORMAT}" = "JSONL" ]; then
        # JSONL格式 (padding only)
        DATA_ARGS+=(
            --dataset-type JSONL
            --tokenizer-type SFTTokenizer
            --sft-tokenizer-prompt-format qwen3
            --dataloader-type cyclic
            --legacy-tokenizer
        )
    else
        # MMAP格式 (支持packing和padding)
        DATA_ARGS+=(--dataset-type MMAP)
        # packing模式：添加position-ids重置参数
