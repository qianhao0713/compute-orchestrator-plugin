...
if [[ $SEQ_LEN_MODE == "32k" ]] && [[ $WORLD_SIZE -ge 8 ]]; then
    ...
    PRESET_MP_PP0_LAYERS=6
    PRESET_MP_PP_LAST_LAYERS=4
...
elif [[ $SEQ_LEN_MODE == "8k" ]] && [[ $WORLD_SIZE -ge 8 ]]; then
    PRESET_TP=1
    PRESET_PP=8
    PRESET_EP=8
    PRESET_ETP=1
    PRESET_CP=1
    PRESET_SEQ_LEN=8192
    PRESET_MP_PP0_LAYERS=10
    PRESET_MP_PP_LAST_LAYERS=12
    PRESET_LOAD_OPTI=false
    if [[ $WORLD_SIZE -eq 32 ]]; then
        PRESET_GLOBAL_BATCH_SIZE=256
    elif [[ $WORLD_SIZE -eq 16 ]]; then
...
elif [[ $SEQ_LEN_MODE == "4k" ]] && [[ $WORLD_SIZE -ge 8 ]]; then
    PRESET_TP=4
    PRESET_PP=8
    PRESET_EP=8
    PRESET_ETP=1
    ...
    PRESET_LOAD_OPTI=false
    if [[ $WORLD_SIZE -eq 32 ]]; then
        PRESET_GLOBAL_BATCH_SIZE=2048
    elif [[ $WORLD_SIZE -eq 16 ]]; then
        PRESET_GLOBAL_BATCH_SIZE=1024
    elif [[ $WORLD_SIZE -eq 8 ]]; then
        PRESET_GLOBAL_BATCH_SIZE=512
    fi
fi
...
if [[ -n "${PRESET_MP_PP0_LAYERS}" ]]; then
    MP_PP0_LAYERS=${MP_PP0_LAYERS:-${PRESET_MP_PP0_LAYERS}}
    MP_PP_LAST_LAYERS=${MP_PP_LAST_LAYERS:-${PRESET_MP_PP_LAST_LAYERS}}
fi
...
export DATASET_PATH=${DATASET_PATH:-${STORAGE_PATH}/dataset/pile-llama/pile-llama_text_document}
