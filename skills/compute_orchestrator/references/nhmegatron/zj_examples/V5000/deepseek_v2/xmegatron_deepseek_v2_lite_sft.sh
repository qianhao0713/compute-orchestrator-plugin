...
if [[ $WORLD_SIZE -eq 2 ]]; then
    # 模型层数
    export NUM_LAYERS=${NUM_LAYERS:-27}
    export DENSE_LAYERS=${DENSE_LAYERS:-1}
    export MOE_LAYERS=${MOE_LAYERS:-26}
    # 并行配置
    export TP=${TP:-4}
    export PP=${PP:-1}
    export EP=${EP:-16}
    ...
    export PRETRAIN_CHECKPOINT_PATH=${PRETRAIN_CHECKPOINT_PATH:-"/mnt/lhycpfs/lhy/mbridge_ckpt/DeepSeek-V2-Lite-mcore"}
    ...
    echo "Unsupported WORLD_SIZE: ${WORLD_SIZE}"
    exit 1
fi
...
export TOKENIZER_PATH=${TOKENIZER_PATH:-"/mnt/lhycpfs/lhy/model/deepseek-v2-tokenizer"}
if [[ $SFT -ne 1 ]]; then
    export DATA_PATH=${DATA_PATH:-"/mnt/lhycpfs/lhy/dataset/deepseek-v2/deepseek_v2_dataset_text_document"}
elif [[ $PACKING -eq 1 ]]; then
    export DATA_PATH=${DATA_PATH:-"/mnt/lhycpfs/lhy/dataset/deepseek-v2/deepseek_v2_sft_packing_32k_text_document"}
else
...
# 设置保存路径
MODEL_CONFIG_PATH="$TRAINING_PATH/examples/deepseek_v2"
...
SAVE_PATH="$OUTPUT_DIR/checkpoint/mcore-deepseek-v2-lite-16B"
...
MODEL_ARGS=(
    --model-config deepseek_v2_lite_config
    --moe-grouped-gemm
...
TRAINING_ARGS=(
    ...
    --no-load-rng
    $([[ $CPT_CONTINUE -ne 1 ]] && echo "--no-load-optim")
    $([[ ${SAVE_CKPT:-1} -eq 1 ]] && echo "--save $SAVE_PATH")
    $([[ $CPT_CONTINUE -eq 1 || $PRETRAIN_CHECKPOINT_PATH != "" ]] && echo "--load ${PRETRAIN_CHECKPOINT_PATH:-$SAVE_PATH}")
