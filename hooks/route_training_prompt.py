...
def build_hook_output(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if explicit_gpu:
        context = (
            ...
            "If the current GPU cluster is compatible and sufficient, keep it "
            "without switching even when another cluster is more suitable. "
            "Otherwise rank compatible clusters by task suitability, then "
            "call get_available_clusters with required_gpu_count and prefer a "
            "compatible cluster with capacitySufficient=true; if none has "
            "enough cards, choose the most suitable and use the fixed queue question. "
            "Inspect executable code, estimate resources, and inspect current "
            "container resources before execution or expansion. Before every "
            "smoke test or workload launch, call get_resource_status. If current "
            "resources suffice but provisioning is true, use the fixed queued-switch "
            "risk AskUserQuestion; Continue directly allows this task and Cancel "
            "forbids it. Do not poll or call ensure_resource in that branch. "
            "If GPU-96G is a "
